                                           ...
                                           hal_pcd_handle_t *hpcd)
{
  ...
  if (_ux_device_stack_control_request_process(transfer_request) == UX_SUCCESS)
  {
    ...
    HAL_PCD_SetEndpointTransmit(hpcd, 0x00U, UX_NULL, 0U);
  }
}
...
/*    HAL_PCD_SetEndpointTransmit           Transmit data                 */
/*    HAL_PCD_SetEndpointReceive            Receive data                  */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
/*    STM32 HAL Driver                                                    */
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        /* Receive data.  */
        HAL_PCD_SetEndpointReceive(hpcd,
...
VOID HAL_PCD_DataInStageCallback(hal_pcd_handle_t *hpcd, uint8_t ep_num)
{
  ...
  if ((ep_num & 0xF) != 0)
    ed =  &dcd_stm32 -> ux_dcd_stm32_ed_in[ep_num & 0xF];
  else
#endif /* defined(UX_DEVICE_BIDIRECTIONAL_ENDPOINT_SUPPORT) */
    ed =  &dcd_stm32 -> ux_dcd_stm32_ed[ep_num & 0xF];
  ...
  if (ep_num == 0U)
  {
    ...
    if (ed -> ux_dcd_stm32_ed_state == UX_DCD_STM32_ED_STATE_DATA_TX)
    {
      ...
      /* Arm Status transfer.  */
      HAL_PCD_SetEndpointReceive(hpcd, 0, 0, 0);
          ...
          endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize)
      {
        ...
        if (transfer_request -> ux_slave_transfer_request_force_zlp ==  UX_TRUE)
        {
          ...
          HAL_PCD_SetEndpointTransmit(hpcd,
  ...
  else
  {
        ...
        transfer_request -> ux_slave_transfer_request_requested_length)
    {
      ...
      /* Arm a ZLP packet on IN.  */
      HAL_PCD_SetEndpointTransmit(hpcd, ep_num, 0, 0);
...
VOID HAL_PCD_DataOutStageCallback(hal_pcd_handle_t *hpcd, uint8_t ep_num)
{
  ...
  if (ep_num == 0U)
  {
    ...
    if (ed -> ux_dcd_stm32_ed_state == UX_DCD_STM32_ED_STATE_DATA_RX)
    {
      ...
      transfer_length = HAL_PCD_EP_GetRxCount(hpcd, ep_num);
...
VOID HAL_PCD_ResetCallback(hal_pcd_handle_t *hpcd)
{
  ...
  device_speed = (UINT)HAL_PCD_GetDeviceSpeed(hpcd);
...
VOID HAL_PCD_IsoINIncompleteCallback(hal_pcd_handle_t *hpcd, uint8_t ep_num)
{
      ...
      (endpoint->ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION) != 0)
  {
    ...
    HAL_PCD_SetEndpointTransmit(dcd_stm32 -> pcd_handle,
                                endpoint->ux_slave_endpoint_descriptor.bEndpointAddress,
...
VOID HAL_PCD_IsoOUTIncompleteCallback(hal_pcd_handle_t *hpcd, uint8_t ep_num)
{
      ...
      (endpoint->ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION) == 0)
  {
    ...
    /* Incomplete, discard data and retry.  */
    HAL_PCD_SetEndpointReceive(dcd_stm32 -> pcd_handle,
                               endpoint->ux_slave_endpoint_descriptor.bEndpointAddress,
                               endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_data_pointer,
                               endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_requested_length);
  }
...
/*  FUNCTION                                                RELEASE       */
/*                                                                        */
/*    HAL_PCD_LpmCallback                                  PORTABLE C     */
/*                                                            6.x.xx      */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Chaoqiong Xiao, Microsoft Corporation                               */
...
/*    DATE              NAME                      DESCRIPTION             */
/*                                                                        */
/*  xx-xx-xxxx     Chaoqiong Xiao           Initial Version 6.x.xx        */
/*                                                                        */
/**************************************************************************/
VOID HAL_PCD_LpmCallback(hal_pcd_handle_t *hpcd, hal_pcd_lpm_active_status_t lpm_status)
{
  ...
  if (_ux_system_slave -> ux_system_slave_change_function != UX_NULL)
  {
    ...
    /* Inform the application if a callback function was programmed.  */
    _ux_system_slave -> ux_system_slave_change_function(UX_DCD_STM32_DEVICE_LPM);
  }
}
...
/*  FUNCTION                                                RELEASE       */
/*                                                                        */
/*    HAL_PCD_BcdCallback                                  PORTABLE C     */
/*                                                            6.x.xx      */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Chaoqiong Xiao, Microsoft Corporation                               */
...
VOID HAL_PCD_BcdCallback(hal_pcd_handle_t *hpcd, hal_pcd_bcd_port_type_t port_type)
{
  ...
  if (_ux_system_slave -> ux_system_slave_change_function != UX_NULL)
  {
    ...
    /* Inform the application if a callback function was programmed.  */
    _ux_system_slave -> ux_system_slave_change_function(UX_DCD_STM32_DEVICE_BCD);
  }
